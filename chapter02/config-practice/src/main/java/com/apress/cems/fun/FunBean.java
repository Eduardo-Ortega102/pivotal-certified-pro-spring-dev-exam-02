...
public class FunBean implements InitializingBean, DisposableBean {
    ...
    @PostConstruct
    private void init() {
        logger.info("Stage 3: Calling @PostConstruct");
    }
    ...
    private void beanInitialization() {
        logger.info("Stage 5: Calling @Bean(initMethod)");
    }

    @PreDestroy
    private void preDestroy() {
        logger.info("Stage 6: Calling preDestroy");
    }

    @Override
    public void destroy() throws Exception {
        logger.info("Stage 7: Calling destroy");
    }

    private void beanDestruction() {
        logger.info("Stage 8: Calling @Bean(destroyMethod)");
    }
}
